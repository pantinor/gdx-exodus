...
public class FoodVendor extends BaseVendor {
    ...
    public FoodVendor(Vendor vendor, Context context, Party.PartyMember member) {
    ...
    public boolean nextDialog() {
        ...
        switch (state) {
            ...
            case BUY_FOOD:
                ...
                member.getPlayer().food = haveCount + currentCount * food.getQuantity();
                member.getPlayer().adjustGold(-food.getPrice() * currentCount);
    ...
    public boolean checkCanPayFood() {
        ...
        if (food.getPrice() * currentCount > member.getPlayer().gold) {
            displayToScreen("You can only afford " + Math.round(member.getPlayer().gold / food.getPrice()) + " packs.");
        } else {
            ret = true;
        }
