...
import exodus.Constants.GuildItemType;
import exodus.Context;
import exodus.Party;

public class GuildVendor extends BaseVendor {

    public GuildVendor(Vendor vendor, Context context, Party.PartyMember member) {
        ...
        switch (state) {
            ...
            case BUY_ITEM:
                ...
                switch (currentSelectedItem.getGuildItemType()) {
                    case gem:
                        member.getPlayer().gems += currentSelectedItem.getQuantity();
                        break;
                    case key:
                        member.getPlayer().keys += currentSelectedItem.getQuantity();
                        break;
                    case powder:
                        member.getPlayer().powder += currentSelectedItem.getQuantity();
                        break;
                    case torch:
                        member.getPlayer().torches += currentSelectedItem.getQuantity();
                        break;
                }
