...
public abstract class BaseVendor {
    ...
    public BaseVendor(Vendor vendor, Context context, PartyMember member) {
        this.vendor = vendor;
    ...
    public boolean checkCanPay(Item item) {
        ...
        if (member.getPlayer().gold > item.getPrice() * 2) {
    ...
    public boolean checkCanPay(Item item, int count) {
        ...
        if (item.getPrice() * count > member.getPlayer().gold) {
