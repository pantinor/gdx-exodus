...
public class ArmorVendor extends BaseVendor {
    ...
    public ArmorVendor(Vendor vendor, Context context, PartyMember member) {
    ...
    public boolean nextDialog() {
        ...
        switch (state) {
            ...
            case BUY_ITEM:
                haveCount = member.getPlayer().armors[currentSelectedItem.getArmorType().ordinal()];
                member.getPlayer().armors[currentSelectedItem.getArmorType().ordinal()] = haveCount + currentCount;
            ...
            case WAIT_SELL_INPUT:
                ...
                for (Item i : vendor.getVendorClass().getItemCatalog()) {
                    int c = member.getPlayer().armors[i.getArmorType().ordinal()];
                    if (c > 0) {
    ...
    public void setResponse(String input) {
        ...
        if (state == ConvState.ASK_BUY_SELL) {
            ...
            if (item == null) {
                ...
                int count = member.getPlayer().armors[item.getArmorType().ordinal()];
                if (count > 1) {
            ...
            int haveCount = member.getPlayer().armors[currentSelectedItem.getArmorType().ordinal()];
            if (count <= haveCount) {
                currentCount = count;
