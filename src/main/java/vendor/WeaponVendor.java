...
public class WeaponVendor extends BaseVendor {
    ...
    public WeaponVendor(Vendor vendor, Context context, Party.PartyMember member) {
    ...
    public boolean nextDialog() {
        ...
        switch (state) {
            ...
            case BUY_ITEM:
                haveCount = member.getPlayer().weapons[currentSelectedItem.getWeaponType().ordinal()];
                member.getPlayer().weapons[currentSelectedItem.getWeaponType().ordinal()] = haveCount + currentCount;
                member.getPlayer().adjustGold(-currentSelectedItem.getPrice() * currentCount);
            ...
            case WAIT_SELL_INPUT:
                ...
                for (Item i : vendor.getVendorClass().getItemCatalog()) {
                    int c = member.getPlayer().weapons[i.getWeaponType().ordinal()];
                    if (c > 0) {
            ...
            case SELL_ITEM:
                haveCount = member.getPlayer().weapons[currentSelectedItem.getWeaponType().ordinal()];
                member.getPlayer().weapons[currentSelectedItem.getWeaponType().ordinal()] = haveCount - currentCount;
                member.getPlayer().gold = member.getPlayer().gold + (currentSelectedItem.getPrice() / 2) * currentCount;
    ...
    public void setResponse(String input) {
        ...
        if (state == ConvState.ASK_BUY_SELL) {
            ...
            if (item == null) {
                ...
                int count = member.getPlayer().weapons[item.getWeaponType().ordinal()];
                if (count > 1) {
            ...
            int haveCount = member.getPlayer().weapons[currentSelectedItem.getWeaponType().ordinal()];
            if (count <= haveCount) {
                currentCount = count;
