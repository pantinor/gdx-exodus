...
public class HealerService extends BaseVendor {
    ...
    public HealerService(Vendor vendor, Context context, PartyMember member) {
    ...
    public boolean nextDialog() {
        ...
        switch (state) {
            ...
            case WAIT_GIVE_BLOOD:
                ...
                break;
            case GIVE_BLOOD:
                member.getPlayer().health = member.getPlayer().health - 100;
            ...
            case PERFORM_SERVICE:
                if (currentCount <= 0 || currentCount > context.getParty().getMembers().size()) {
                    currentCount = 1;
                }
                PartyMember pm = context.getParty().getMember(currentCount - 1);
                if (pm == null) {
                    break;
                }
                if (pm.heal(currentSelectedItem.getHealType())) {
                    ...
                    displayToScreen(pm.getPlayer().name + ", thou art not in need of such service.");
                }
    ...
    public void setResponse(String input) {
        ...
        if (state == ConvState.WAIT_NEED_HELP || state == ConvState.ANYTHING_ELSE) {
            if (input.startsWith("y")) {
                ...
                if (member.getPlayer().health >= 400) {
